import { defineStore } from "pinia";
import { wordApi } from "../services/api";
import type { Word, EnglishLevel, WordsState } from "../types";

export const useWordsStore = defineStore("words", {
  state: (): WordsState => ({
    currentWord: null,
    words: [],
    selectedLevel:
      (localStorage.getItem("selectedLevel") as EnglishLevel) || null,
    loading: false,
    error: null,
  }),

  getters: {
    hasCurrentWord: (state): boolean => !!state.currentWord,
    wordsByLevel:
      (state) =>
      (level: string): Word[] => {
        return state.words.filter((word) => word.level === level);
      },
  },

  actions: {
    setLevel(level: EnglishLevel): void {
      this.selectedLevel = level;
      localStorage.setItem("selectedLevel", level);
    },

    async fetchWordsByLevel(level: EnglishLevel, perPage = 20) {
      this.loading = true;
      this.error = null;

      try {
        const response = await wordApi.getByLevel(level, perPage);
        this.words = response.data || [];
        return this.words;
      } catch (error: any) {
        this.error = error.message;
        console.error("Error fetching words:", error);
        throw error;
      } finally {
        this.loading = false;
      }
    },

    async fetchRandomWord(level: EnglishLevel | null = null): Promise<Word> {
      const targetLevel = level || this.selectedLevel;

      if (!targetLevel) {
        throw new Error("No level selected");
      }

      this.loading = true;
      this.error = null;

      try {
        const response = await wordApi.getRandom(targetLevel);
        // Handle Laravel Resource format { data: {...} }
        const word = response.data || (response as any);
        this.currentWord = word;
        return word;
      } catch (error: any) {
        this.error = error.message;
        console.error("Error fetching random word:", error);
        throw error;
      } finally {
        this.loading = false;
      }
    },

    async fetchWord(id: number): Promise<Word> {
      this.loading = true;
      this.error = null;

      try {
        const response = await wordApi.getWord(id);
        // Handle Laravel Resource format { data: {...} }
        const word = response.data || (response as any);
        this.currentWord = word;
        return word;
      } catch (error: any) {
        this.error = error.message;
        console.error("Error fetching word:", error);
        throw error;
      } finally {
        this.loading = false;
      }
    },

    async fetchAndStoreWord(
      wordText: string,
      level: EnglishLevel
    ): Promise<Word> {
      this.loading = true;
      this.error = null;

      try {
        const response = await wordApi.fetchAndStore(wordText, level);
        // Handle Laravel Resource format { data: {...} }
        const word = response.data || (response as any);
        this.currentWord = word;
        this.words.push(word);
        return word;
      } catch (error: any) {
        this.error = error.message;
        console.error("Error fetching and storing word:", error);
        throw error;
      } finally {
        this.loading = false;
      }
    },

    clearCurrentWord(): void {
      this.currentWord = null;
    },

    clearError(): void {
      this.error = null;
    },
  },
});
